...
export default class Player extends Phaser.Physics.Matter.Sprite{
    ...
    constructor(data,socket,label){
        let {scene,x,y,texture,frame}=data;
        super(scene.matter.world,x,y,texture,frame,{label:label,circleRadius:80});
    ...
    create(scene){
        ...
        scene.input.on("pointermove",function(pointer){
            if(!self.mining && self.events) {
                self.angle=Phaser.Math.RadToDeg(Phaser.Math.Angle.Between(self.x, self.y, pointer.worldX, pointer.worldY));
                self.socket.emit("newAngle",self.angle);
            }   
            if(self.events)self.degree=self.angle;         
        });
        scene.input.on("pointerdown",(pointer)=>{
            if(pointer.leftButtonDown() && self.events){
                this.shootButton=true;          
            } 
        }); 
        scene.input.on("pointerup",()=>{
            if(self.events)this.shootButton=false;
        });             
    ...
    update(scene){        
        ...
        if(this.inputKeys.handgun.isDown && this.activeWeapon!="handgun"){
            this.activeWeapon="handgun";
            this.socket.emit("weaponChange",this.activeWeapon);
            this.shootRatio=2.5;
            this.canShoot=0;
        ...
        if((Math.abs(this.velocity.x)>0.1 || Math.abs(this.velocity.y)>0.1)){
            if(!this.shooting && !this.reloading && !this.mining)this.anims.play(this.activeWeapon+"_move",true);
            this.socket.emit("playermove",this.body.position);
        }else if(!this.shooting && !this.reloading && !this.mining){
            this.socket.emit("playeridle");
        ...
        if(this.canMine && this.inputKeys.interact.isDown){
            ...
            if(isOre && this.inRange(hitObject,100) && !this.mining){
                ...
                this.socket.emit("playerMining",false);
                speed=0;
                this.angle=0;
                this.mining=true;
        ...
        if(this.mining && this.inputKeys.interact.isUp){            
            this.socket.emit("playerMining",true);
            this.mining=false;
            this.canMine=true;
            if(this.oreObj!=null){
        ...
        if(this.canShoot>=this.shootRatio){
            ...
            this.on("animationstart",()=>{ 
                if(this.bullet && this.shooting){
                    this.bullet=false;
                    var target=new Phaser.Math.Vector2(this.scene.input.mousePointer.worldX,this.scene.input.mousePointer.worldY);
                    this.socket.emit("PlayerShooting",target);
    ...
    buildTurret(ironUsed,scene,turretBaseUsed){
        this.socket.emit("TurretBuilt",turretBaseUsed);
